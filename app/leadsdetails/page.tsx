...
export default function Home() {
  ...
  // ===== IST Scheduling Helpers =====

  // Timezone (India)
  const CA_TZ = "Asia/Kolkata"; // kept the same variable name to avoid touching UI code

  // Window + slot config (IST)
  const WORK_START_HOUR = 10; // 10:30 AM start
  const WORK_START_MINUTE = 30;
  const WORK_END_HOUR = 19; // 7:30 PM end
  const WORK_END_MINUTE = 30;

  const SLOT_MINUTES = 15; // time-step shown in picker
  const DURATION_MINUTES = 30; // length of scheduled slot

  // UTC -> India local (for showing)
  const toPickerLocal = (d: Date | null) =>
    d ? utcToZonedTime(d, CA_TZ) : null;

  // India local -> UTC (for storing/state)
  ...
  // Round UP to next SLOT_MINUTES (India time)
  ...
  // Start of "today" (India tz) for minDate
  const startOfTodayCA = () => {
    const n = nowCA(); // n is IST-local
    // Construct a JS Date at local midnight for the IST calendar day
    return new Date(n.getFullYear(), n.getMonth(), n.getDate(), 0, 0, 0, 0);
  };

  // Build a time on the same calendar day (India local)
  ...
  // Daily bounds (India local)
  const windowStartCA = (dCA: Date) =>
    atHM(dCA, WORK_START_HOUR, WORK_START_MINUTE); // 10:30
  const windowEndCA = (dCA: Date) => atHM(dCA, WORK_END_HOUR, WORK_END_MINUTE); // 19:30

  // Next valid START within window (defaults & today's earliest)
  // Allows picking up to 19:30; if "now" >= 19:30, move to tomorrow 10:30
  const nextValidStartInWindowCA = (base: Date = nowCA()) => {
    const today = new Date(base);
    const wStart = windowStartCA(today);
    const wEnd = windowEndCA(today);

    if (base < wStart) return wStart; // before 10:30 -> 10:30
    if (base >= wEnd) {
      // Move to tomorrow's 10:30
      const tomorrow = addMinutes(atHM(today, 0, 0), 24 * 60);
  ...
  // Formatter for payload (India local) — keeps your MM-dd-yyyy h:mma for UI consistency
          ...
          <div className="w-full min-h-auto  text-white p-4">
            ...
            <Formik
              initialValues={{
                ...
                location: "online",
                description: "",
                start_at: defaultStart, // stored as UTC
                end_at: defaultEnd, // stored as UTC
              }}
              validationSchema={Yup.object({
                    ...
                    <div className="w-full md:col-span-2">
                      ...
                      <div className="w-full relative mb-4">
                        ...
                        <DatePicker
                          ...
                          dateFormat="MM-dd-yyyy h:mma" // keep UI format unchanged
                          placeholderText="MM-dd-yyyy hh:mmam/pm"
                          className="w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4 bg-black text-white"
                          popperClassName="custom-datepicker"
                          minDate={startOfTodayCA()}
                          filterTime={(candidate) => {
                            // Constrain selectable times to IST window [10:30, 19:30]
                            const candCA = toPickerLocal(candidate)!; // candidate in IST
                            const wStart = windowStartCA(candCA); // 10:30
                            const wEnd = windowEndCA(candCA); // 19:30
                            let earliest = wStart;

                            // If picking on "today" (IST), the earliest is the next rounded slot after now
                            if (isSameDay(candCA, nowCA())) {
                              const nextSlot = roundUpToSlot(nowCA());
                              if (nextSlot > wEnd) return false; // past the day's window
                              earliest = nextSlot < wStart ? wStart : nextSlot;
                            }

                            return (
                              candCA.getTime() >= earliest.getTime() &&
                              candCA.getTime() <= wEnd.getTime() // allow up to 19:30
                            );
                          }}
                          dayClassName={(date) => {
              ...
              <Formik
                ...
                initialValues={{
                  ...
                  start_at:
                    (taskEditObject?.start_at
                      ...
                      : defaultStart) || defaultStart, // stored as UTC
                  end_at:
                    (taskEditObject?.end_at
                      ...
                      : defaultEnd) || defaultEnd, // stored as UTC
                ...
                onSubmit={async (values, { setSubmitting }) => {
                  ...
                  if (isEditing) {
                    ...
                    // ----- CREATE payload (kept for completeness) -----
                    const payload = {
                      lead_id: leadId,
                      assigned_agent_id: data?.agent?.id || "",
                      details: values.description || "",
                  ...
                  return (
                    <form onSubmit={handleSubmit} noValidate>
                      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:justify-between mb-4 sm:mb-6">
                        ...
                        {/* ====== COMMON FIELDS (shown in BOTH, and ONLY these in EDIT) ====== */}
                          ...
                          <div className="w-full relative mb-4">
                            <div className="w-full relative mb-4">
                              ...
                              <DatePicker
                                /* SHOW as India time */
                                selected={toPickerLocal(values.start_at)}
                                /* TAKE as India time -> store UTC and set end = start + 30 */
                                onChange={(date: Date | null) => {
                                  const utcStart = fromPickerLocal(date);
                                  setFieldValue("start_at", utcStart);
                                  if (utcStart)
                                ...
                                showTimeSelect
                                timeFormat="h:mma"
                                timeIntervals={SLOT_MINUTES} // 15-min steps
                                dateFormat="MM-dd-yyyy h:mma"
                                placeholderText="MM-dd-yyyy hh:mmam/pm"
                                ...
                                /* Disable past days */
                                minDate={startOfTodayCA()}
                                /* HIDE times outside 10:30am–7:30pm IST, and hide today's past times */
                                filterTime={(candidate: Date) => {
                                  const candCA = toPickerLocal(candidate)!; // candidate in IST

                                  // Window bounds for the candidate's local day
                                  const wStart = windowStartCA(candCA); // 10:30
                                  const wEnd = windowEndCA(candCA); // 19:30

                                  // Default earliest = 10:30
                                  let earliest = wStart;

                                  // If picking on "today" (IST), earliest is the next rounded slot within window
                                  if (isSameDay(candCA, nowCA())) {
                                    const nextSlot = roundUpToSlot(nowCA());
                                    if (nextSlot > wEnd) return false; // no slots left today
                                    earliest =
                                      nextSlot < wStart ? wStart : nextSlot;
                                  }

                                  // Allow only [earliest, 19:30]
                                  return (
                                    candCA.getTime() >= earliest.getTime() &&
                                    candCA.getTime() <= wEnd.getTime()
                                  );
                                }}
                              />
